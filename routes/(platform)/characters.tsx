...
export async function handler(
  ...
  const personajes: Character[] = await Promise.all(charactersAPI.data.map(async (c: Character) => {
    const buscarFavorito = await collectionFavoritos.findOne({username: ctx.state.username, id: c.id});
    if(buscarFavorito) {
      return {
